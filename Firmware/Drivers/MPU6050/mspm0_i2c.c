...
static int mspm0_i2c_has_timed_out(unsigned long start)
{
    unsigned long now;

    mspm0_get_clock_ms(&now);
    return ((now - start) >= I2C_TIMEOUT_MS) ? 1 : 0;
}
                     ...
                     unsigned char const *data)
{
    ...
    if (mspm0_i2c_wait_controller_idle() != 0)
        return -1;
