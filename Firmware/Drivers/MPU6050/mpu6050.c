...
#define MPU6050_FIFO_DRAIN_LIMIT  (8U)
...
void MPU6050_NotifyDataReadyFromIsr(void)
{
    /*
     * GPIO 中断只记录“DMP 有新数据”。
     * 真正的 I2C/DMP FIFO 读取放在主循环里做，避免中断里长时间阻塞编码器计数。
     */
    hal.new_gyro = 1U;
}
...
{
    if ((hal.dmp_on == 0U) || (hal.new_gyro == 0U)) {
        return 0U;
    }

    hal.new_gyro = 0U;
    return (Read_Quad() == 0) ? 1U : 0U;
}
...
int Read_Quad(void)
{
    ...
    */
    ...
    do
    {
        ...
        if (fifo_drain_count >= MPU6050_FIFO_DRAIN_LIMIT) {
            break;
        }
    }while(more);
    ...
    pitch  = asinf(-2.0f * q1 * q3 + 2.0f * q0 * q2) * 57.29578f;
